...
class AgentRessource(baseAgent.BaseAgent):
	"""
	...
	def __init__(self, model, path='logger/', model_name='model', method_name="method", method=None, load_model=False,
		...
		self.method = method(
			model = model,
        	input_dim=[(4,self.map_size,self.map_size),(30,7),(11,),(13,)],
	...
	def train(self, obs_new, obs, action, reward):
		...
		feat = AgentRessource.get_feature_screen(obs)
		# Store the reward
	...
	def step(self, obs):
		...
		super(AgentRessource, self).step(obs)
		...
		if act[0] == 0:
			if actions.FUNCTIONS.Move_screen.id in obs.observation['available_actions']:
				...
				position = AgentRessource.prediction_to_position([act[1]], dim = self.map_size)
			...
			position1 = AgentRessource.prediction_to_position([act[1]], dim = self.map_size)
			position2 = AgentRessource.prediction_to_position([act[2]], dim = self.map_size)
	...
	def get_feature_screen(obs):
		...
		return [np.array(mapp),multi_select,np.array(obs.observation.player),np.array(obs.observation.score_cumulative)]
