...
class Agent(base_agent.BaseAgent):
	...
	def __init__(self, model=None, rl=None):
		...
		if model is not None:
			...
			self.epoch = self.rl.load(model)


	def train(self, obs_new, obs, action, reward):
		# Train the agent
		reward = -1 if reward == 0 else 1
		feat = self.get_feature_screen(obs, features.SCREEN_FEATURES.player_relative)
		# Store the reward
	...
	def step(self, obs):
		...
		if actions.FUNCTIONS.Move_screen.id in obs.observation['available_actions']:
			...
			positions = self.prediction_to_position([act])
