...
class AgentAttMap(baseAgent.BaseAgent):
	...
	def __init__(self, model, path='logger/', model_name='model', method_name="method", method=None, load_model=False,
		...
		self.method = method(
			model = model,
        	input_dim=[(5,self.map_size,self.map_size),(4,self.minimap_size,self.minimap_size),(3,7)],
        	output_dim=[3,self.map_size*self.map_size,self.minimap_size*self.minimap_size],
        	pi_lr=pi_lr,
        	gamma=gamma,
	...
	def step(self, obs):
		...
		if act[0] == 0:
			if actions.FUNCTIONS.Move_screen.id in obs.observation['available_actions']:
				...
				position = AgentAttMap.prediction_to_position([act[1]], dim = self.map_size)
			...
			if actions.FUNCTIONS.move_camera.id in obs.observation['available_actions']:
				# Convert the prediction into positions
				position = AgentAttMap.prediction_to_position([act[2]],  dim = self.minimap_size)
				# Get a random location on the map
